...
Two things had to be fixed before any of it worked. The first boards each need four wire links before their relay chains can conduct at all, which revision B will design out ([hardware #44](https://github.com/origin89hq/hardware/issues/44)). Then the sequence still failed for most of a day, on a reworked board and an untouched one alike: the five-wire cable had been crimped with ferrules inside the terminals, and a crimp cannot bite through a ferrule. Three conductors happened to conduct, so the board powered up and `RUN` arrived, but `KICK` never did and the watchdog never released. Recrimping without ferrules fixed it.
